...
cmd({ 
    ...
    try { 
        if (!q) return await reply("*𝐏lease provide a YouTube URL or Song Name.*");

        let apiUrl = `https://ochinpo-helper.hf.space/yt?query=${encodeURIComponent(q)}`;
        
        let response = await fetch(apiUrl);
        let data = await response.json();
        
        if (!data.success || !data.audio || !data.thumbnail || !data.title) {
            return reply("Failed to fetch the audio. Please try again later.");
        }
...
╔══════════════════❒
║ ⿻ *ᴛɪᴛʟᴇ:*  ${data.title}
║ ⿻ *ᴅᴜʀᴀᴛɪᴏɴ:*  ${data.duration || 'Unknown'}
║ ⿻ *ᴠɪᴇᴡs:*  ${data.views || 'Unknown'}
║ ⿻ *ᴀᴜᴛʜᴏʀ:*  ${data.channel || 'Unknown'}
║ ⿻ *ʟɪɴᴋ:*  ${data.url || 'Unavailable'}
╚══════════════════❒
> *ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴘʀɪɴᴄᴇ ᴛᴇᴄʜ*`;
