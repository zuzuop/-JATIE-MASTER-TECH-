...
cmd({
    ...
    try {
        ...
        if ((m.type === 'imageMessage') || isQuotedImage) {
            ...
            // Get the caption from the original message
            const caption = isQuotedImage ? m.quoted.caption : m.caption;

            // Send the image back instead of converting to sticker
            return conn.sendMessage(from, { image: { url: nameJpg }, caption: caption || 'Here is your downloaded image.' }, { quoted: mek });
        } 
        // If the message is a sticker or quoted sticker
        else if (isQuotedSticker) {
            const nameWebp = getRandom('.webp');
        ...
        // If the message is a video or quoted video
            ...
            // Get the caption from the original message
            const caption = isQuotedVideo ? m.quoted.caption : m.caption;

            // Send the video back to the user
            return conn.sendMessage(from, { video: { url: nameMp4 }, caption: caption || 'Here is your downloaded video.' }, { quoted: mek });
        }
        // If the message is an audio or quoted audio
        else if (isQuotedAudio) {
            const nameAudio = getRandom('.mp4');
            ...
            // Get the caption from the original message
            const caption = isQuotedAudio ? m.quoted.caption : m.caption;

            // Send the audio back to the user
            return conn.sendMessage(from, { audio: { url: nameAudio }, mimetype: 'audio/mp4', caption: caption || 'Here is your downloaded audio.' }, { quoted: mek });
        } else {
            return await reply(imgmsg);
        }
    } catch (e) {
